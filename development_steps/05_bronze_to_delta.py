...
def cast(credits_sch):
    ...
    # silver_bucket = s3resource.Bucket('tmdb-silver')
    # count = silver_bucket.objects.filter(Prefix=f"{current_ts}/cast/_delta_log")
    # if len(list(count)) == 0:
    #     credits_cast.write.mode("overwrite").format("delta").save(f"s3a://tmdb-silver/{current_ts}/cast")
    # elif len(list(count))>0:
    #     read_last_cast = spark.read.format("delta").load(f"s3a://tmdb-silver/{current_ts}/cast")
...
def credits_organizer():
    ...
    for table in credits_tables:
        yesterday_table = spark.read.format("delta").load(f"s3a://tmdb-silver/{table.__name__}/")
        print(f"{table} icin calisiyor")
        for i in file_names:
            if i == file_names[0]:
                credits_sch = read_credits(i)
                credits_part_new = table(credits_sch)[0]
                credits_part_merged = yesterday_table.union(credits_part_new)
                print(f"first {credits_part_merged.count()}")
            else:
                credits_sch = read_credits(i)
                credits_part_new = table(credits_sch)[0]
                credits_part_merged = credits_part_merged.union(credits_part_new)
                print(f"new {credits_part_new.count()}")
                print(f"merged {credits_part_merged.count()}")
        write_delta_to_silver(credits_part_merged, table(credits_sch)[1])
...
def movies_organizer():
    ...
    for table in movies_tables:
        yesterday_table = spark.read.format("delta").load(f"s3a://tmdb-silver/{table.__name__}/")
        print(f"yesterday table {yesterday_table.count()}")
        print(f"{table} icin calisiyor")
        for i in file_names:
            if i == file_names[0]:
                movies_raw = read_movies(i)
                movies_part_new = table(movies_raw)[0]
                movies_part_merged = yesterday_table.union(movies_part_new)
                print(f"yesterday + new {movies_part_merged.count()}")
            else:
                movies_raw = read_movies(i)
                movies_part_new = table(movies_raw)[0]
                movies_part_merged = movies_part_merged.union(movies_part_new)
                print(f"new {movies_part_new.count()}")
                print(f"merged {movies_part_merged.count()}")
        write_delta_to_silver(movies_part_merged, table(movies_raw)[1])
